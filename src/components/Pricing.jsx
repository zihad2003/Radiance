    ...
    return (
        ...
        >
            <motion.div
                ...
                className="bento-card bg-[#0A0A0A] border border-white/10 rounded-[3rem] w-full max-w-6xl max-h-full overflow-hidden flex flex-col md:flex-row shadow-2xl relative"
            >
                <button onClick={onClose} className="absolute top-8 right-8 p-3 hover:bg-white/10 rounded-full transition-colors z-[60] text-white">
                    <X size={24} />
                </button>
                ...
                <div className={`hidden lg:flex w-80 flex-col p-12 text-white overflow-hidden relative ${pkg.theme}`}>
                    ...
                    <div className="relative z-10 h-full flex flex-col">
                        ...
                        <div className="mt-12 space-y-4">
                            {[1, 2, 3, 4].map(i => (
                                <div key={i} className={`h-1 rounded-full transition-all duration-500 ${step >= i ? 'bg-primary w-full' : 'bg-white/20 w-8'}`} />
                            ))}
                        </div>
                    </div>
                </div>
                ...
                <div className="flex-1 p-8 md:p-16 overflow-y-auto custom-scrollbar bg-[#0A0A0A]">
                    ...
                    {step === 1 && (
                        <div className="space-y-10">
                            <div>
                                ...
                                <h3 className="text-4xl font-serif italic text-white">Enhance Your Package</h3>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {ADDONS.slice(0, 8).map(addon => (
                                    <button
                                        ...
                                        className={`p-6 rounded-3xl border text-left flex justify-between items-center transition-all ${selections.includes(addon.id) ? 'border-primary bg-primary/10' : 'border-white/5 bg-white/5 hover:border-white/20'
                                            }`}
                                    >
                                        <div>
                                            <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400">{addon.name}</p>
                                            <p className="text-sm font-black text-white">+৳{addon.price.toLocaleString()}</p>
                                        </div>
                                        <div className={`p-2 rounded-full ${selections.includes(addon.id) ? 'bg-primary text-black' : 'bg-white/10 text-gray-500'}`}>
                    ...
                    {step === 2 && (
                        <div className="space-y-10">
                            <div>
                                ...
                                <h3 className="text-4xl font-serif italic text-white">Select Date & Time</h3>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                                <div className="space-y-6">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-gray-500">Scheduled Date</label>
                                    <input
                                        type="date"
                                        className="w-full p-5 bg-white/5 rounded-2xl border-none outline-none font-bold text-white focus:ring-2 ring-primary/20"
                                    />
                                ...
                                <div className="space-y-6">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-gray-500">Preferred Time</label>
                                    <div className="grid grid-cols-2 gap-3">
                                        {["10:00 AM", "12:30 PM", "03:00 PM", "06:00 PM"].map(t => (
                                            <button
                                                key={t}
                                                onClick={() => setBookingDetails({ ...bookingDetails, time: t })}
                                                className={`py-4 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${bookingDetails.time === t ? 'bg-primary text-black shadow-lg shadow-primary/30' : 'bg-white/5 text-gray-400 hover:bg-white/10'}`}
                                            >
                                                {t}
                                            </button>
                                        ))}
                    ...
                    {step === 3 && (
                        <div className="space-y-10">
                            <div>
                                ...
                                <h3 className="text-4xl font-serif italic text-white">Client Information</h3>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    <input type="text" placeholder="FULL NAME" className="w-full p-5 bg-white/5 rounded-2xl outline-none border border-transparent focus:border-primary/20 text-xs font-bold text-white placeholder:text-white/20" />
                                </div>
                                <div className="space-y-2">
                                    <input type="tel" placeholder="PHONE NUMBER (+880)" className="w-full p-5 bg-white/5 rounded-2xl outline-none border border-transparent focus:border-primary/20 text-xs font-bold text-white placeholder:text-white/20" />
                                </div>
                                <div className="col-span-full">
                                    <textarea placeholder="SPECIAL REQUESTS (ALLERGIES, CONCERNS, ETC.)" rows={4} className="w-full p-5 bg-white/5 rounded-3xl outline-none border border-transparent focus:border-primary/20 text-xs font-bold text-white placeholder:text-white/20" />
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    {step === 4 && (
                        <div className="space-y-10">
                            <div>
                                ...
                                <h3 className="text-4xl font-serif italic text-white">Secure Checkout</h3>
                            </div>

                            <div className="bg-white/5 text-white rounded-[3rem] p-10 flex flex-col md:flex-row gap-12 items-center border border-white/10">
                                <div className="flex-1 text-center md:text-left">
                                    <p className="text-[10px] font-black uppercase tracking-[0.4em] text-white/40 mb-2">Total Due Today (50%)</p>
                                    <p className="text-6xl font-black text-white">৳{(totalPrice / 2).toLocaleString()}</p>
                                ...
                                <div className="flex flex-col gap-4 w-full md:w-auto">
                                    <button className="px-12 py-5 bg-primary text-black rounded-2xl font-black uppercase tracking-widest text-[10px] flex items-center justify-center gap-3 hover:scale-105 transition-all shadow-2xl">
                    ...
                    {/* Navigation Footer */}
                    <div className="mt-16 pt-8 border-t border-white/10 flex justify-between items-center text-white">
                        <div className="flex flex-col">
                            <span className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">Est. Investment</span>
                            <span className="text-2xl font-black text-white">৳{totalPrice.toLocaleString()}</span>
                        </div>
                        <div className="flex gap-4">
                            {step > 1 && (
                                <button locale="en" onClick={prevStep} className="px-8 py-4 bg-white/5 text-gray-400 rounded-2xl font-black uppercase tracking-widest text-[10px] hover:bg-white/10 transition-all">
                                    Back
                                </button>
                            )}
                            <button
                                onClick={step === 4 ? onClose : nextStep}
                                className="px-10 py-4 bg-white text-black rounded-2xl font-black uppercase tracking-widest text-[10px] hover:bg-primary hover:text-white transition-all shadow-xl flex items-center gap-2"
                            >
...
const CustomizerModal = ({ onClose }) => {
    ...
    return (
        <motion.div
            ...
            className="fixed inset-0 z-[110] flex items-center justify-center bg-black/90 backdrop-blur-3xl p-4 md:p-12 overflow-y-auto"
        >
            <motion.div className="bento-card bg-[#0A0A0A] border border-white/10 rounded-[3rem] w-full max-w-6xl p-8 md:p-16 relative flex flex-col md:flex-row gap-12 shadow-3xl text-white">
                        ...
                        {ADDONS.map(addon => (
                            <button
                                ...
                                className={`p-6 rounded-[2rem] border-2 text-left transition-all flex justify-between items-center ${selections.includes(addon.id)
                                    ...
                                    : 'border-white/5 bg-white/5 hover:border-white/20 shadow-sm'
                                    }`}
                            >
                                <div>
                                    <p className="text-xs font-bold uppercase tracking-widest mb-1 text-white">{addon.name}</p>
                                    <p className="text-primary font-black">+৳{addon.price.toLocaleString()}</p>
                                </div>
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center ${selections.includes(addon.id) ? 'bg-primary text-black' : 'bg-white/10 text-gray-500'}`}>
                ...
                <div className="w-full md:w-96 bg-white/5 rounded-[3rem] p-10 flex flex-col shadow-inner border border-white/5">
                    <h4 className="text-[10px] font-black uppercase tracking-[0.4em] text-gray-500 mb-8">Summary Breakdown</h4>
                    ...
                    <div className="pt-8 border-t-2 border-dashed border-white/20 mt-8">
                        <p className="text-[10px] font-black uppercase tracking-[0.4em] text-gray-500 mb-2">Total Investment</p>
                        <p className="text-5xl font-black text-white mb-8">৳{totalPrice.toLocaleString()}</p>
                        <button className="w-full bg-primary text-black py-5 rounded-3xl font-black uppercase tracking-[0.2em] text-[10px] shadow-2xl hover:scale-105 transition-transform active:scale-95">
